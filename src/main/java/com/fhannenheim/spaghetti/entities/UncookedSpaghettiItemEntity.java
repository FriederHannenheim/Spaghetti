...
public class UncookedSpaghettiItemEntity extends ItemEntity {
    ...
    public UncookedSpaghettiItemEntity(World worldIn, double x, double y, double z) {
        super(worldIn, x, y, z);
        createTag();
    }

    public UncookedSpaghettiItemEntity(World worldIn, double x, double y, double z, ItemStack stack) {
        super(worldIn, x, y, z, stack);
        createTag();
    }
    void createTag(){
        CompoundNBT tag = this.getItem().getOrCreateTag();
        tag.putInt("cook_time",600);
    ...
    public void tick() {
        ...
        if (inBlock.getBlock() == Blocks.CAULDRON) {
            Spaghetti.LOGGER.info("In cauldron");
        ...
        if (this.getItem().getOrCreateTag().getInt("cook_time") <= 0) {
            ...
            world.playSound(null, this.getPosition(), SoundEvents.BLOCK_CAMPFIRE_CRACKLE, SoundCategory.PLAYERS, 2, .5f);
            this.remove();
        }
    }
    */
